const NWS_SERVICE = {
    getIcon: (text, isDay = true, prob = 0) => {
        let t = text.toLowerCase();
        const r = "icons/";
        
        // BUG FIX: If precip < 35%, force cloudy icon
        if (prob < 35 && (t.includes("showers") || t.includes("rain") || t.includes("thunderstorm"))) {
            return isDay ? r + "mostly_cloudy_day.png" : r + "mostly_cloudy_night.png";
        }
    ...
    async fetchFullData(lat, lon) {
        try {
            ...
            return {
                ...
                sun: { rise: sunData.daily.sunrise, set: sunData.daily.sunset }
            };
        } catch (e) { return null; }
    }
};